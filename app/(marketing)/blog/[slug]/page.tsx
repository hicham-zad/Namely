...
export default async function BlogPostPage({
  ...
  return (
    <div style={{ background: "var(--surface)", minHeight: "100vh", paddingBottom: "4rem" }}>
      ...
      {/* Header */}
      <div style={{ background: "var(--surface-el)", padding: "3rem 1.5rem 2rem", borderBottom: "1px solid var(--border-lt)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          {/* Breadcrumb */}
          <nav aria-label="Breadcrumb" style={{ fontSize: "0.8rem", color: "var(--text3)", marginBottom: "1.25rem" }}>
            <ol style={{ display: "flex", gap: "0.4rem", alignItems: "center", listStyle: "none", padding: 0, margin: 0 }}>
              <li><Link href="/" style={{ color: "var(--text3)", textDecoration: "none" }}>Home</Link></li>
              <li aria-hidden="true">›</li>
              <li><Link href="/blog" style={{ color: "var(--text3)", textDecoration: "none" }}>Blog</Link></li>
              <li aria-hidden="true">›</li>
              <li style={{ color: "var(--text)", fontWeight: 500 }}>{post.title}</li>
            </ol>
          </nav>
          ...
          <h1 style={{ fontFamily: "var(--font-outfit)", fontSize: "clamp(1.6rem, 4vw, 2.5rem)", fontWeight: 900, color: "var(--text)", lineHeight: 1.2, marginBottom: "1rem", letterSpacing: "-0.02em" }}>
            {post.title}
          </h1>

          <p style={{ color: "var(--text2)", fontSize: "1.05rem", lineHeight: 1.6, marginBottom: "1.25rem" }}>
            {post.description}
          </p>

          <div style={{ display: "flex", gap: "1.25rem", fontSize: "0.8rem", color: "var(--text3)" }}>
      ...
      <div className="blog-post-layout" style={{ maxWidth: 1100, margin: "0 auto", padding: "3rem 1.5rem", display: "grid", gridTemplateColumns: "1fr", gap: "3rem" }}>
        ...
        <aside>
          {/* CTA card */}
          <div style={{ background: "var(--surface-el)", border: "1.5px solid var(--border)", borderRadius: 20, padding: "1.5rem", marginBottom: "2rem", textAlign: "center" }}>
            <div style={{ fontSize: "2rem", marginBottom: "0.75rem" }}>👶</div>
            <h3 style={{ fontFamily: "var(--font-outfit)", fontWeight: 800, color: "var(--text)", marginBottom: "0.5rem", fontSize: "1rem" }}>
              Find your perfect baby name
            </h3>
            <p style={{ fontSize: "0.82rem", color: "var(--text2)", marginBottom: "1rem", lineHeight: 1.5 }}>
          ...
          {relatedPosts.length > 0 && (
            <div>
              <h3 style={{ fontFamily: "var(--font-outfit)", fontWeight: 800, color: "var(--text)", fontSize: "0.9rem", marginBottom: "1rem", paddingBottom: "0.5rem", borderBottom: "2px solid var(--border-lt)" }}>
                Related Articles
              </h3>
                ...
                {relatedPosts.map((r) => (
                  <li key={r.slug}>
                    <Link href={`/blog/${r.slug}`} style={{ textDecoration: "none", display: "flex", gap: "0.75rem", alignItems: "flex-start" }}>
                      ...
                      <span style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", lineHeight: 1.35 }}>{r.title}</span>
                    </Link>
                  </li>
                ))}
              </ul>
      ...
      {/* Bottom CTA */}
      <section style={{ background: "var(--surface-el)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "3rem 1.5rem", textAlign: "center" }}>
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-outfit)", fontWeight: 900, color: "var(--text)", fontSize: "clamp(1.25rem, 3vw, 1.6rem)", marginBottom: "0.5rem" }}>
