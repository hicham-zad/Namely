...
export default async function BlogIndexPage({
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section style={{ background: "linear-gradient(180deg, #f8fafc 0%, #fff 100%)", padding: "3rem 1.5rem 2rem", textAlign: "center", borderBottom: "1px solid #f1f5f9" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <h1 style={{ fontFamily: "var(--font-outfit)", fontSize: "clamp(1.75rem, 4vw, 2.5rem)", fontWeight: 900, color: TEXT_DARK, marginBottom: "0.75rem", letterSpacing: "-0.02em" }}>
            Baby Name Blog
          </h1>
      ...
      <div style={{ maxWidth: 1152, margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        <nav aria-label="Filter by topic" style={{ marginBottom: "2rem" }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
            <Link href="/blog" style={{ padding: "0.4rem 1rem", borderRadius: 999, fontSize: "0.82rem", fontWeight: 600, textDecoration: "none", background: !topic ? TEXT_DARK : "#f1f5f9", color: !topic ? "#fff" : "#6b7280" }}>
              All
            </Link>
            {topics.map((t) => (
              <Link key={t.topic} href={`/blog?topic=${encodeURIComponent(t.topic)}`}
                style={{ padding: "0.4rem 1rem", borderRadius: 999, fontSize: "0.82rem", fontWeight: 600, textDecoration: "none", background: topic === t.topic ? topicColor(t.topic) : "#f1f5f9", color: topic === t.topic ? TEXT_DARK : "#6b7280" }}>
                {t.topic} ({t.count})
              </Link>
            ))}
          </div>
            ...
            {posts.map((post) => (
              <li key={post.slug}>
                <Link href={`/blog/${post.slug}`} style={{ textDecoration: "none", display: "block" }}>
                  <article className="blog-card" style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 20, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.03)", transition: "box-shadow 0.2s, transform 0.2s" }}>
                    ...
                    <div style={{ padding: "1.25rem" }}>
                      ...
                      <h2 style={{ fontFamily: "var(--font-outfit)", fontSize: "1rem", fontWeight: 800, color: TEXT_DARK, lineHeight: 1.35, marginBottom: "0.5rem" }}>
                        {post.title}
                      </h2>
                      <p style={{ fontSize: "0.82rem", color: "#6b7280", lineHeight: 1.5, marginBottom: "0.75rem", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                        {post.description}
                      </p>
          ...
          <nav aria-label="Pagination" style={{ display: "flex", justifyContent: "center", gap: "0.5rem", marginTop: "2rem" }}>
            {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map((p) => (
              <Link key={p} href={`/blog?page=${p}${topic ? `&topic=${encodeURIComponent(topic)}` : ""}`}
                style={{ padding: "0.4rem 0.9rem", borderRadius: 8, fontSize: "0.85rem", fontWeight: 600, textDecoration: "none", background: p === page ? LOGO_BLUE : "#f1f5f9", color: p === page ? TEXT_DARK : "#6b7280" }}
